...
def Transaction(mqtt_data, S_Blkchain_ID):
    w3 = Web3(Web3.HTTPProvider("http://" + config['ETHEREUM_Node2']['host'] + ":" + config['ETHEREUM_Node2']['port']))
    w3.parity.personal.unlockAccount(w3.toChecksumAddress(config['ETHEREUM_Node2']['miner_account']), config['ETHEREUM_Node2']['miner_passwd'], 0)
...
def Verification(hashcode): 
    w3 = Web3(Web3.HTTPProvider("http://" + config['ETHEREUM_Node1']['host'] + ":" + config['ETHEREUM_Node1']['port']))
    w3.parity.personal.unlockAccount(w3.toChecksumAddress(config['ETHEREUM_Node1']['miner_account']), config['ETHEREUM_Node1']['miner_passwd'], 0)
