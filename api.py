...
cfgpath = path.split('BIM_P_API_2N')[0] + 'BIM_P_API_2N/config.ini'
...
def upload():
    ...
    cur.execute('''SELECT * FROM BIMP2N.Project_Name WHERE Project_id = '%s';''' %Project_id)
    data = cur.fetchall()
    
    if(len(data) == 0):
        INSERT = '''INSERT INTO BIMP2N.Project_Name(Project_id,
        ...
        cur.execute('''UPDATE BIMP2N.Project_Name SET Project_update_times = '%s' WHERE Project_id = '%s';''' %(int(data[0][1]) + 1, Project_id))
    ...
    Project_element_hashcode = Transaction(eth_data, config['ETHEREUM_Node2']['user'])
    bc_end_time = time.time()
    Project_bc_end_time = d('%Y') + '/' + d('%m') + '/' + d('%d')+'/' + d('%H') + ':' + d('%M') + ':' + d('%S') + '.' + d('%f')
    Project_bc_time = bc_end_time - bc_start_time

    INSERT = '''INSERT INTO BIMP2N.Project_Information(Project_id,
                                                    Project_user_id,
                                                    Object_id,
                                                    Element_id,
                                                    Element_location,
    ...
    cur.execute('''UPDATE BIMP2N.Project_Information SET Project_check_in_db_time = '%s' WHERE Project_id = '%s';''' %(Project_db_time, Project_id))
    mysql.commit()
...
def location():
    ...
    if(len(data_text_name) == 0):
        ...
        cur.execute('''SELECT * FROM BIMP2N.Project_Information WHERE %s LIKE '%s';''' %(data_text_name[0], '%'+data_text_value[0]+'%'))
        data = cur.fetchall()
        ...
        if (data_text_name[0] == "Element_id"):
            ...
            cur.execute('''UPDATE BIMP2N.Project_Information SET Project_id_co_time = '%s' WHERE Element_id = '%s';''' %(Get_time, Element_id))
            mysql.commit()

        return json.dumps(data_out), status.HTTP_200_OK
    
    elif(len(data_text_name) == 2):
        cur.execute('''SELECT * FROM BIMP2N.Project_Information WHERE (%s LIKE '%s') AND (%s LIKE '%s');'''
...
def upload_file():
    ...
    Check_element_id = cur.execute("SELECT * FROM BIMP2N.Project_Information WHERE Element_id = '%s'"%(Element_id))
    ...
    cur.execute("SELECT * FROM BIMP2N.Project_Information WHERE Element_id = '%s'"%(Element_id))
    data = cur.fetchall()
    Project_id = data[0][0]
    ...
    cur.execute('''UPDATE BIMP2N.Project_Information SET Project_element_path = '%s', Project_element_size = '%s',
...
def download_file():
    ...
    if(len(data) == 0):
        ...
        cur.execute('''UPDATE BIMP2N.Project_Information SET Project_source_ip = '%s' WHERE Element_id = '%s';''' %(Download_file_time, Element_id))
        mysql.commit()
...
def check_out():
    ...
    for i in range(len(request_data['Element_id'])):
        ...
        cur.execute('''SELECT * FROM BIMP2N.Project_Information WHERE Element_id = '%s';''' %((request_data['Element_id'][i])))
        data = cur.fetchall()
        ...
        if(data[0][10] != "1"):
            ...
            Check_out_element_hashcode = Transaction(eth_data, config['ETHEREUM_Node2']['USER'])
            bc_end_time = time.time()
            ...
            cur.execute('''UPDATE BIMP2N.Project_Information SET Check_out_status = '%s', Check_out_time = '%s',
            ...
            cur.execute('''UPDATE BIMP2N.Project_Information SET Check_in_status = '%s', Project_check_out_db_time = '%s' WHERE Element_id = '%s';''' %('0', check_out_db_time, request_data['Element_id'][i]))
            mysql.commit()
            data_json = {}
...
def Checkin_verification():
    ...
    cur.execute('''SELECT * FROM BIMP2N.Project_Information WHERE Project_check_in_element_hashcode = '%s';''' %(Check_in_hash))
    data = cur.fetchall()
    ...
    if(len(data) == 0):
        ...
        if (res2 == res1):
            ...
            INSERT = '''INSERT INTO BIMP2N.Project_hash_verify(
                                                    Project_id,
                                                    Element_id,
                                                    Check_in_status,
                                                    Project_element_hashcode,
...
def Checkout_verification():
    ...
    cur.execute('''SELECT * FROM BIMP2N.Project_Information WHERE Project_check_out_element_hashcode = '%s';''' %(Check_out_hash))
    data = cur.fetchall()
